...
class ConsumeWebserviceJob extends AbstractJob implements JobInterface
{
    ...
    private function callServiceFunction(
        ...
        // Si se provee un token, se establece como cookie en el cliente SOAP.
        if ($token !== null) {
            $soap->__setCookie('TOKEN', $token);
        }
        ...
        // Si la respuesta es un string que no corresponde a XML, se envuelve en
        // un tag <data> para poder cargarlo como XmlDocument.
        if (is_string($responseBody) && !str_starts_with(ltrim($responseBody), '<')) {
